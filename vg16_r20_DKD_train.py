...
save_path = './checkpoint/dcgd_dkd_0_4_model_1'
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        outputs_student_sigle = net(inputs)
        outputs_teacher = net_teacher(inputs)

        loss_1 = dkd_loss(outputs_student_sigle, outputs_teacher, targets, alpha=1.0, beta=4.0, temperature=args.temperature)
        loss_2 = criterion(outputs_student_sigle, targets)
    ...
    print('Train Loss: {:.4f}  single Acc: {:.4f}'.format(epoch_loss, epoch_acc_sigle))
